...
class ManagerStoriesController extends Controller
{
        ...
        try {
            if(Story::where('slug', $request->slug)->first()) {
                ...
                if($request->has('source')){
                    ...
                    $data['image'] = Storage::url($full_path);
                }
    ...
    public function slugExist(Request $request){
        $success = false;
        if($request->has('submit') && $request->has('name') && $request->has('author')){
            $story = Story::where('name', $request->name)->where('author', $request->author)->first();
            if ($story == null) {
                $success =  true;
            } else {
                if($request->submit == 'edit' && $request->has('id') && $request->id==$story->id) {
                    $success =  true;
                }
            }
